...
def test_detect_no_findings(
    ...
    mock_ner_detector.detect.assert_called_once_with("some text")
...
def test_detect_with_findings_and_deduplication(
    ...
    mock_ner_detector.detect.return_value = [
        {"type": "ID", "value": "123", "label": "NER_ID", "start": 10}, # Duplicate value and type, different label
        {"type": "kontakt", "value": "b@c.com", "label": "EMAIL_NER", "start": 20}
    ]
    ...
    detector = SensitiveDataDetector()
    results = detector.detect("email a@b.com then 100PLN and id 123 also b@c.com")
    ...
    # NER:   ("123", "ID") -> duplicate, ("b@c.com", "kontakt")
    ...
    # 4. {"type": "ID", "value": "123", "label": "NER_ID", "start": 10} (NER) - will be dropped if "start" is identical to previous for same (value, type)
    # 5. {"type": "kontakt", "value": "b@c.com", "label": "EMAIL_NER", "start": 20} (NER)
...
def test_detect_sorting_and_deduplication_order_matters(
    ...
    mock_ner_detector.detect.return_value = [
        {"type": "ID", "value": "123", "label": "NER_ID_EARLY", "start": 5}
    ]
    mock_regex_detector.detect.return_value = [
